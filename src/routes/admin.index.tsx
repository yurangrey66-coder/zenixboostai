...
import { Users, DollarSign, Megaphone, Rocket, TrendingUp } from "lucide-react";
...
function AdminDashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [profiles, payments, todayPayments, ads, pending] = await Promise.all([
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Visão geral do Zenix Boost</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Stat icon={Users} label="Usuários totais" value={stats.users} hint={`${stats.activeUsers} ativos`} />
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-border bg-gradient-card p-5">
          <h3 className="font-display font-semibold">Status dos usuários</h3>
